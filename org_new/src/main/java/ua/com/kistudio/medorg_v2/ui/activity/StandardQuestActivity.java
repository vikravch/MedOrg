...
*/
	...
	protected void showMenu()
	{
		...
		try {
				...
				{
					// str = str + ' ' + s;
					Log.i(Params.LOG_TAG, str.substring(0, s) + ' ' + s + ' ' + str.length() + ' ' + "num=" + num);
					if ( str.length() > s ) 
						{
		...
		for ( num=0; num<mGroupsNames.length; num++)
			Log.i(Params.LOG_TAG, "groups_names[" + num + "] = " + mGroupsNames[num]);
		...
		(
				new AdapterView.OnItemClickListener()
				{
					public void onItemClick( AdapterView<?> parent, View itemClicked, int position, long id )
					{
						...
						Log.i(Params.LOG_TAG, id + ", " + position + " -> " + strText);
